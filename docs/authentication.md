...
| **Cursor** | **The Cursor CLI's own login.** `cursor-agent login` stores a session in Cursor's config; rn-agent runs the CLI and never reads that credential. A `CURSOR_API_KEY` is accepted for CI ([CLI auth](https://cursor.com/docs/cli/reference/authentication)). | `Auth: Cursor CLI session`. No credential is copied, scraped or re-stored |
