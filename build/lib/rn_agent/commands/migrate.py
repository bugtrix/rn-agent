...
class MigrateCommand(AgentCommand[MigrateAnalysis, MigratePlan]):
    ...
    def execute(self, plan: MigratePlan) -> None:
        agent = self.context
        # A dry run writes nothing, so there is nothing to undo and nothing to
        # protect: the git gates guard *changes*. Requiring a clean repository
        # to preview a plan would make the wizard's "Analyze" step useless in
        # exactly the situation it is most wanted - before you commit anything.
