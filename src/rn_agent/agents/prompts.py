...
#: The honesty rules, shared by every task. Deliberately separate from the
#: "reply with JSON" instruction: the interactive terminal wants prose, and a
#: prompt that demands both is a prompt the model has to guess at.
SYSTEM_RULES = """\
...
def chat_messages(
    *,
    ...
    """A conversational turn, with the project facts and recent history.

    History is passed through verbatim rather than summarised: the developer can
    see what they typed, and a summary is another place to introduce a claim
    nobody made.
    """
    messages = [
        Message.system(f"{SYSTEM_RULES}\n\n{project_brief(project, rules)}\n\n{CHAT_CONTRACT}")
    ]
    messages.extend(history)
    files = f"\n\n{_files_block(context)}" if context else ""
    messages.append(Message.user(f"{question}{files}"))
    return messages
